...
from myweather.weathermapkey import key
import pytest
import json
...
def test_in_riga():
    assert get_weather_data("Riga", key) is not None, \
        "Type of response is not none while using the key"

def test_type_of_res():
    assert type(get_weather_data("Riga", key)) is str, \
        "Type of response is not none while using the key"

def test_args_error():
    assert get_weather_data('', key) is None, \
...
def test_coords_dim():
    assert len(json.loads(get_weather_data('Riga', key)).get('coord')) == 2, \
        "Dimension is 2: lon and lat"

def test_temp_type():
    assert type(json.loads(get_weather_data('Riga', key)).get('feels_like')) is float, \
